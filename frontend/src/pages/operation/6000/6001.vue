    ...
    <div class="calc_page" style="background-color: #EDEFF2;">
        <a-row style="margin: 0 -6px">
                ...
                <a-card :title="$t('baseQueryParam')" :headStyle='{ "font-weight": "bolder" }' style="margin-bottom: 24px" :bordered="false"
                    ...
                    <div class="baseQueryParam">
                        <a-form>
                            ...
                            <a-form-item :label="$t('singleTankVolume')" :labelCol="{ span: 6 }"
                                :wrapperCol="{ span: 18 }">
                            ...
                            <a-form-item :label="$t('singleTankVolume')" :labelCol="{ span: 6 }"
                                :wrapperCol="{ span: 18 }">
                                <a-input v-model="singleTankVolume2" rows="4" :placeholder="$t('singleTankVolume')"
                                    :suffix="$t('singleUnit2')" :disabled="true" />
                            </a-form-item>
                        </a-form>
                    </div>
                </a-card>
            ...
            <a-col style="padding: 14px 6px" :xl="8" :lg="24" :md="24" :sm="24" :xs="24">
                <a-card :title="$t('processDesignCompute')" :headStyle='{ "font-weight": "bolder" }' style="margin-bottom: 0px" :bordered="false"
                    :body-style="{ padding: 2 }">
                    <div class="baseQueryParam">
                        <a-form style="margin-bottom: 24px">
                            <a-form-item class="calc" :label="$t('contactTime')" style="margin-top: 5px"
                            ...
                            <a-form-item class="calc" :label="$t('flowLength')" :labelCol="{ span: 6 }"
                                :wrapperCol="{ span: 18 }">
                                <a-input-group compact>
                                    <a-input v-model="flowLength" style="width: 50%;" rows="4"
                                        :placeholder="$t('flowLength')" :disabled="true" />
                                    <a-input v-model="roundingFlowLength" style="width: 50%;" rows="4"
                                        :placeholder="$t('flowLength')" :suffix="$t('flowLengthUnit')"
                                        :disabled="false" />
                                </a-input-group>

                            </a-form-item>
                            <a-form-item class="calc" :label="$t('division')" :labelCol="{ span: 6 }"
                                :wrapperCol="{ span: 18 }">
                                <a-input v-model="division" rows="4" :placeholder="$t('division')" />
                            </a-form-item>
                            <a-form-item class="calc" :label="$t('partitionWallThickness')" :labelCol="{ span: 6 }"
                                :wrapperCol="{ span: 18 }">
                                <a-input v-model="partitionWallThickness" rows="4"
                                    :placeholder="$t('partitionWallThickness')"
                                    :suffix="$t('partitionWallThicknessUnit')" />
                            </a-form-item>
                            <a-form-item class="calc" :label="$t('poolLength')" :labelCol="{ span: 6 }"
                                :wrapperCol="{ span: 18 }">
                                <a-input v-model="poolLength" rows="4" :placeholder="$t('poolLength')"
                                    :suffix="$t('poolLengthUnit')" :disabled="true" />
                            </a-form-item>
                            <a-form-item class="calc" :label="$t('poolWidth')" :labelCol="{ span: 6 }"
                                :wrapperCol="{ span: 18 }">
                                <a-input v-model="poolWidth" rows="4" :placeholder="$t('poolWidth')"
                                    :suffix="$t('poolWidthUnit')" :disabled="true" />
                            </a-form-item>
                            <br>
                            <a-form-item class="calc" :label="$t('perCell')" :labelCol="{ span: 6 }"
                                :wrapperCol="{ span: 18 }">
                                <a-input-group compact>
                                    <a-input v-model="poolLength" style="width: 33%;" rows="8"
                                        :placeholder="$t('poolLength')" :suffix="$t('poolLengthUnit')"
                                        :disabled="true" />
                                    <a-input v-model="roundingFlowLength" style="width: 33%;" rows="8"
                                        :placeholder="$t('poolWidth')" :suffix="$t('poolWidthUnit')" :disabled="true" />
                                    <a-input v-model="contactPoolDepth" style="width: 34%;" rows="8"
                                        :placeholder="$t('contactPoolDepth')" :suffix="$t('contactPoolDepthUnit')"
                                        :disabled="true" />
                                </a-input-group>
                            </a-form-item>
                        </a-form>
        ...
        <div id="testApp" hidden="true">
            ...
            <div>
                ...
                <p><span>（2）{{ $t('totalChangeFactor') }} k= {{ this.totalChangeFactor }} {{ $t('designScaleUnit')
                        }}</span></p>
                <p><span>（3）{{ $t('singleTankVolume') }} X1=Q/24*k， X1= {{ this.singleTankVolume1 }} {{
                        $t('singleUnit1') }}, 取 {{ this.singleTankVolume2 }} {{ $t('singleUnit2') }}</span></p>
                <br>
                <h4>工艺设计计算：</h4>
                <p><span>（1）{{ $t('contactTime') }} A1= {{ this.contactTime }} {{ $t('contactTimeUnit') }}</span></p>
                <p><span>（2）{{ $t('totalPoolCapacity') }} A2=X1*A1/60， A2= {{ this.totalPoolCapacity }} {{
                    $t('totalPoolCapacityUnit') }}</span></p>
                <p><span>（3）{{ $t('disinfectiontank') }} A3= {{ this.disinfectiontank }} {{ $t('disinfectiontankUnit')
                        }}</span></p>
                <p><span>（4）{{ $t('everyPoolCapacity') }} A4=A2/A3， A4= {{ this.everyPoolCapacity }} {{
                    $t('everyPoolCapacityUnit') }}</span></p>
                <p><span>（5）{{ $t('contactPoolDepth') }} A5= {{ this.contactPoolDepth }} {{ $t('contactPoolDepthUnit')
                        }}</span></p>
                <p><span>（6）{{ $t('singlecellWidth') }} A6= {{ this.singlecellWidth }} {{ $t('singlecellWidthUnit')
                        }}</span></p>
                <p><span>（7）{{ $t('flowLength') }} A7=A4/A5/A6， A7= {{ this.flowLength }} {{ $t('flowLengthUnit') }}, 取
                        {{ this.roundingFlowLength }} {{ $t('flowLengthUnit') }}</span></p>
                <p><span>（8）{{ $t('division') }} A8= {{ this.division }}</span></p>
                <p><span>（9）{{ $t('partitionWallThickness') }} A9= {{ this.partitionWallThickness }} {{
                    $t('partitionWallThicknessUnit') }}</span></p>
                <p><span>（10）{{ $t('poolLength') }} A10=A7/A8， A10= {{ this.poolLength }} {{ $t('poolLengthUnit')
                        }}</span></p>
                <p><span>（11）{{ $t('poolWidth') }} A11=A8*A6+(A8-1)*A9， A11= {{ this.poolWidth }} {{ $t('poolWidthUnit')
                        }}</span></p>
                <br>
                <p><span>（12）{{ $t('perCell') }} A12=A10*A11*A5，A12= {{ this.poolLength }} {{ $t('poolLengthUnit') }} *
                        {{ this.poolWidth }} {{ $t('poolWidthUnit') }} * {{ this.contactPoolDepth }} {{
                            $t('contactPoolDepthUnit') }} </span></p>
            </div>
...
export default {
    ...
    methods: {
        ...
        getDimensions() {
            return parseFloat(this.poolLength) + "m x " + this.poolWidth + "m x " + this.contactPoolDepth + "m";
        }
    },
    computed: {
